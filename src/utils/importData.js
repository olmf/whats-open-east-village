...
function removeOverlap(rows) {
    ...
    return coordRows.map(row => {
        ...
        if (overlapped.length > 1) {
            ...
            row.coordinates = [lng1 + index * 0.00002, lat1 + index * 0.00003]
        }
        return row
    })
}
...
async function importData(file, store) {
    ...
    const filterRows = rows.filter(({Latitude, Longitude, Status, Chain}) => +Latitude && +Longitude && Status === 'Opened' && Chain != 'true')
